...
test.serial(
  "My Account - Delete Account (success)",
  async (t) => {
    const genDELETE = sinon
      .stub(HTTPReq, "genDELETE")
      .resolves(true);
    const wrapper = mount(
      VMyAccount,
      newLocalVueAndRouter(),
...
test.serial(
  "My Account - Delete Account (failure)",
  async (t) => {
    const genDELETE = sinon
      .stub(HTTPReq, "genDELETE")
      .resolves(false);
...
test.serial(
  ...
  async (t) => {
    ...
    t.is(
      genPOST.args[0][1],
      WingsStructUtil.stringify(currentUser),
    );
    t.true(message.item.calledOnce);
    t.is(
      message.getMessage(),
